Hooks:OverrideFunction(DOTManager,"add_doted_enemy",function(self,data)
	...
	if should_sync then
		...
		if alive(attacker) and attacker == managers.player:local_player() then
			if dot_info and dot_info.variants and dot_info.variants.poison then
					...
					if dot_range > 0 then
						...
						for i = 1, #nearby_enemies do
							...
							if enemy ~= doted_unit then
								...
								if base and not base.sentry_gun then -- don't apply dot to sentryguns
				...
				dot_info.variants.poison.dot_damage = dot_info.variants.poison.dot_damage * managers.player:upgrade_value("subclass_poison", "weapon_subclass_damage_mul", 1)
			end
		end
